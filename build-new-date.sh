...
if [ -z "$TARGET_DATE" ]; then
    >&2 echo "Must specify a target date as first argument"
    exit 1
fi

CHANNEL_FILE="channel-rust-nightly.toml"
...
dlfile() {
    component="$1"
    triple="$2"
    download "https://static.rust-lang.org/dist/$TARGET_DATE/$component-nightly-$triple.tar.gz"
}
...
get_rust_md5sum() {
    component="$1"
    triple="$2"
    get_md5sum "$component-nightly-$triple.tar.gz"
}

get_rust_sha256sum() {
    component="$1"
    triple="$2"
    get_sha256sum "$component-nightly-$triple.tar.gz"
}
...
download_files() {
    download "https://static.rust-lang.org/dist/$TARGET_DATE/$CHANNEL_FILE"
...
get_copyright_md5sum() {
    for triple in "${RUSTC_TRIPLES[@]}"; do
        echo $triple
        rustcfile=rustc-nightly-${triple}
...
# This corresponds to rust release nightly ${TARGET_DATE}
...
DEPENDS += "rust-bin-cross-\${TARGET_ARCH} (= ${TARGET_DATE})"
LIC_FILES_CHKSUM = "\\
...
RUST_BIN_RECIPE="$ROOT_DIR/recipes-devtools/rust/rust-bin-cross_$TARGET_DATE.bb"
CARGO_BIN_RECIPE="$ROOT_DIR/recipes-devtools/rust/cargo-bin-cross_$TARGET_DATE.bb"
